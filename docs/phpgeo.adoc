...
_phpgeo_ provides unit tests with a quite good coverage. The following command
runs the tests:
...
Or using PHP 7.2 Beta:

----
docker run -it --rm --name phpgeo-phpunit -v "$PWD":/usr/src/phpgeo -w /usr/src/phpgeo php:7.2-rc-cli php vendor/bin/phpunit
----
